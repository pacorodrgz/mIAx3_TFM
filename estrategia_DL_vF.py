...
'''
FUNCIONES DE CONSTRUCCIÓN DEL DF DE PREDICCIONES
'''

def import_modelo(ruta, nombre):
  modelo = keras.models.load_model('{0}/Output/Estudio_Modelos/{1}.h5'.format(ruta, nombre))
  return modelo

def min_max_scaler_datos(df):
  normalizer = preprocessing.MinMaxScaler()
  norm_array = normalizer.fit_transform(df)
  df_norm = pd.DataFrame(norm_array, columns = df.columns, index=df.index)
  return df_norm

def import_dataset(ruta, dateFormat):
  parser = lambda f: pd.datetime.strptime(f, dateFormat)
  df = pd.read_csv("{0}/Output/Estudio_Caracteristicas/dataset.csv".format(ruta), sep = ";", parse_dates = ['Fecha'], date_parser = parser, index_col='Fecha', decimal = ",")
  if 'Y_FmI_Dv_Opt' in df.columns:
    del df['Y_FmI_Dv_Opt']
  
  df = min_max_scaler_datos(df)
  return df

def make_his_predicciones(ruta, nombre, dateFormat):
  modelo = import_modelo(ruta, nombre)
  df_dataset = import_dataset(ruta, dateFormat)
  predicciones = modelo.predict(df_dataset)
  df_predicciones = pd.DataFrame(predicciones, columns = ['Prediccion'], index=df_dataset.index)
...
def make_resultados(df_futuro, df_tesoreria, ruta, iniDate, endDate, modelo, estilo = 'seaborn'):
  ...
  plt.savefig('{0}/Output/Estrategia_DL/Futuro_MiniIbex{1}.png'.format(ruta, periodo))
  plt.close()
  ...
  plt.savefig('{0}/Output/Estrategia_DL/Resultado_{1}_{2}.png'.format(ruta, modelo, periodo))
  plt.close()
...
def export_CSV(df, ruta, nombre, iniDate, endDate):
  periodo = str(iniDate)[:10].replace('-', '') + '-' + str(endDate)[:10].replace('-', '')
  
  df.to_csv(path_or_buf = '{0}/Output/Estrategia_DL/{1}_{2}.csv'.format(ruta, nombre, periodo), sep = ';', float_format = '%.4f', index = False, encoding = 'utf-8')
...
def run(ruta, modelo,
        iniDate = dt.datetime(year=2010,month=6,day=23),
        endDate = dt.datetime(year=2019,month=2,day=22),
        futuroDateFormat = '%d/%m/%Y',
        opcionDateFormat = '%Y-%m-%d',
        datasetDateFormat = '%Y-%m-%d',
  ...
  PrdData = make_his_predicciones(ruta, modelo, datasetDateFormat)
  ...
  for i in FutData.index:
    if FutData.F_Venc[i] == True:
      ...
      desvStrike = PrdData.at[fecVenc, 'Prediccion']
      ...
      if desvStrike < 1.0:
        ...
        print("{}: NO SE OPERA".format(fecVenc))
    else:
      TesData = update_tesoreria(TesData, i)

  LiqData = make_resultados(FutData, TesData, ruta, iniDate, endDate, modelo)

  # Exportamos los resultados a CSV
  export_CSV(FutData, ruta, '_01_fut_mI_{}'.format(modelo), iniDate, endDate)
  export_CSV(OpcData, ruta, '_02_Opc_mI_{}'.format(modelo), iniDate, endDate)
  export_CSV(PosData, ruta, '_03_Pos_{}'.format(modelo), iniDate, endDate)
  export_CSV(TesData, ruta, '_04_Tes_{}'.format(modelo), iniDate, endDate)
